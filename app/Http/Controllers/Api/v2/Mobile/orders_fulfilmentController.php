...
use App\Http\Resources\OrderPickupDelegateCollection;
use App\Http\Resources\OrderDelegateResource;
use App\Http\Resources\SearchResource;
use App\Http\Resources\StatusesorderResource;
use App\Http\Resources\Statusesorder_FulfmientResource;
use App\Http\Resources\PickupDelegateResource;
...
class orders_fulfilmentController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->user_type == 'delegate') {
            // return new OrderDelegateCollection(($user->ordersDelegate()->where('work_type', 4)->paginate(15)));
            return new OrderDelegateCollection(Order::where('delegate_id',$user->id)->where('work_type', 4)->paginate(15));
    ...
    public function pickuporders()
    {
        ...
        if ($user->user_type == 'delegate') {
            return new OrderPickupDelegateCollection((Pickup_order::where('delegate_id',$user->id)->paginate(10)));
    ...
    public function Daily()
    {
        ...
        if ($user->user_type == 'delegate') {
            return new OrderDelegateCollection(($user->ordersDelegate()->where('work_type', 1)->paginate(50)));
    ...
    public function Dailystatus()
    {
        $user = Auth::user();
        if ($user->user_type == 'delegate') {
            $statuses = Statusesorder_FulfmientResource::collection(Status::where('delegate_appear', 1)->where('company_id', $user->company_id)->orderBy('sort', 'ASC')->get());
            if (count($statuses) == 0) {
                return response()->json(
                    [
                        'success' => 0,
    ...
    public function statuses()
    {
        ...
        if ($user->user_type == 'delegate') {
            ...
            $statuses = StatusesResource::collection(Status::where('client_appear', 1)->where('company_id', $user->company_id)->orderBy('sort', 'ASC')->get());
            if (count($statuses) == 0) {
                return response()->json(
                    ['success' => 0,
        ...
        if ($user->user_type == 'delegate') {
            ...
            if(count($result) > 0){
                return response()->json([
                    ...
                    $result = Pickup_order::where('order_id', $request->order_id)
                        ->get();
                
                $result = PickupDelegateResource::collection($result);
    ...
    public function git_orders_status(request $request){
        ...
        if ($user->user_type == 'delegate') {
            ...
            if ($status) {
                ...
                return new OrderDelegateCollection(($user->ordersDelegate()->where('work_type',4)->where('status_id',$status->id)->paginate(10)));
