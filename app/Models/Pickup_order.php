...
class Pickup_order extends Model
{
    use SoftDeletes;

    // protected $dates = ['pickup_date', 'created_at'];
    protected $fillable = [
        'order_id', 'user_id', 'tracking_id', 'company_id', 'sender_city', 'sender_phone', 'sender_address', 'sender_address_2', 'warehouse_id',
        'storage_types', 'size_id', 'work_type', 'status_id', 'delegate_id', 'service_provider_id', 'consignmentNo', 'distance', 'store_address_id', 'delivery_service',
        'pallete_number',
    ];
    ...
    public function pickup_orders_good()
    {
        return $this->hasMany(Pickup_orders_good::class, 'order_id');
    ...
    public function Client_packages_good()
    {
        ...
        return $this->hasMany(Client_packages_good::class, 'order_id');
