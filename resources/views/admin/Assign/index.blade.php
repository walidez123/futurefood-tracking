...
@include('layouts._header-index', ['title' => __("admin_message.Rules for Appointing Delegates") , 'iconClass' => 'fa-users', 'addUrl' => route('AssignOrdersRule.create'), 'multiLang' => 'false'])
              ...
              <tbody>
                ...
                <tr>
                  ...
                  @if (in_array('edit_Orders_rules', $permissionsTitle))
                     <a href="{{route('AssignOrdersRule.edit', $rule->id)}}" title="Edit" class="btn btn-sm btn-primary"
                  ...
                  @if (in_array('delete_Orders_rules', $permissionsTitle))
                      <form class="pull-right" style="display: inline;" action="{{route('AssignOrdersRule.destroy', $rule->id)}}" method="POST">
                          @csrf
                          @method('DELETE')
