...
@section('pageTitle', __('app.floors'))
@section('nav')
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ route('warehouse_floor.store') }}" method="post" class="col-xs-12">
                            @csrf
                            <input type="hidden" name="stand_id" value="{{ $stand_id }}">

                            <div class="col-lg-12">
                                <label>{{ __('app.floor number') }}</label>
                ...
                <div class="col-xs-12">
                    ...
                    <div class="box">
                     ...
                     <th>{{ $branch->latitude }}</th>
                      ...
                      <td>{{ $branch->area }}</td> -->
                                            <td>
                                                <!-- <a aria-disabled="true"  href="{{ route('warehouse_floor.edit', $branch->id) }}" title="Edit" class="btn btn-sm btn-primary" style="margin: 2px;"><i
                            class="fa fa-edit"></i> <span class="hidden-xs hidden-sm">@lang('app.edit')</span></a> -->

                                                <a href="{{ route('warehouse_package.index', ['id' => $branch->id]) }}"
                                                    title="Edit" class="btn btn-sm btn-primary" style="margin: 2px;"><i
                                                        class="fa fa-eye"></i> <span class="hidden-xs hidden-sm">
                                                        @if ($branch->work == 1)
                                                            @lang('app.packages')
                                                        @else
                                                            @lang('app.Shelves')
                                                ...
                                                <form style="display: inline;"
                                                    action="{{ route('warehouse_floor.destroy', $branch->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    @method('DELETE')
