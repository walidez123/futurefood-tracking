...
<div class="col-xs-12 form-group">
    ...
    <div class="">
        <select class="form-control" name="default_status_id" required>
            ...
            @if($status->shop_appear==1 && $client->work==1)
                    <option {{($client->default_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                        {{$status->trans('title')}}</option>
            @elseif($status->fulfillment_appear==1 && $client->work==4)
                <option {{($client->default_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                    {{$status->trans('title')}}</option>
            @elseif($status->restaurant_appear==1 && $client->work==2)
    ...
    <div class="col-xs-12 form-group">
        ...
        <div class="">
            <select class="form-control" name="available_edit_status" required>
                ...
                @foreach ($statuses as $status)
                    @php
                        $oldValue = old('available_edit_status', $client->available_edit_status);
    ...
    <div class="">
        ...
        <option value="">{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                        <option {{($client->userStatus->sort_by_skus_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif
                @endif       
            @endforeach
    ...
    <div class="">
        ...
        <option value="">{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                    <option {{($client->userStatus->pick_process_package_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif      
                @endif       
            @endforeach
    ...
    <div class="">
        ...
        <option value="">{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                    <option {{($client->userStatus->print_waybill_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif      
                @endif      
            @endforeach
    ...
    <div class="">
        ...
        <option value="">{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                    <option {{($client->userStatus->sort_by_city_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif      
                @endif       
            @endforeach
    ...
    <div class="">
        ...
        <option value="">{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                    <option {{($client->userStatus->store_return_shipment_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif      
                @endif        
            @endforeach
    ...
    <div class="">
        ...
        <option value="">{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                    <option {{($client->userStatus->reprocess_return_shipment_status_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif      
                @endif       
            @endforeach
...
<div class="col-xs-6 form-group">
    ...
    <div class="">
        ...
        <option >{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                        <option {{($client->userStatus->shortage_order_quantity_f_stock == $status->id) ? 'selected' : ''}} value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif
                @endif       
            @endforeach
    ...
    <div class="">
        ...
        <option value="">{{__('admin_message.Select')}} {{__('admin_message.status')}}</option>
            @foreach ($statuses as $status)
                @if($client->work==4)
                    @if($status->fulfillment_appear==1)
                        <option {{($client->userStatus->restocking_order_quantity_to_stock == $status->id) ? 'selected' : ''}}  value="{{$status->id}}">{{$status->trans('title')}}</option>
                    @endif
                @endif       
            @endforeach
