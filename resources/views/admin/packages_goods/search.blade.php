                ...
                <div class="col-xs-12">
                    <div class="nav-tabs-custom">
                        <div class="row text-center" style="padding:12px">
                            ...
                            @endif
                            <form action="{{ route('packages_good.clientSearch') }}" method="GET">
                                <input type="hidden" name="type" value="ship">

                                <div class="col-lg-3">
                                    <div class="form-group ">
                                        <label for="to">{{ __('admin_message.Warehouse Clients') }}</label>
                                        <select value="" class="form-control select2 client_warehouse"
                                            name="client_id">
                                            <option value=""> {{ __('admin_message.Select') }}
                                                {{ __('admin_message.Warehouse Clients') }}</option>
                                            @foreach ($clients as $client)
                                                <option
                                                    {{ isset($singelClient) && $singelClient == $client->id ? 'selected' : '' }}
