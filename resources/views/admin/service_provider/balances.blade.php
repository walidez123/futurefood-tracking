...
@section('pageTitle', __('admin_message.service provider balance'))
@section('css')
...
<div class="content-wrapper">
  ...
  @include('layouts._header-index', ['title' => __('admin_message.service provider balance'), 'iconClass' => 'fa-money-bill', 'addUrl' => '', 'multiLang' => 'false'])
      ...
      <div class="col-xs-12">
        <div class="box">
          <div class="box-header">
            <h3 class="box-title"> 
            @if (in_array('add_balance_service_provider', $permissionsTitle))
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <td>
                      ...
                      <a href="{{route('service_provider.transaction', $delegate->id)}}" title="View" class="btn btn-sm btn-info" style="margin: 2px;"><i class="fa fa-bars"></i> <span class="hidden-xs hidden-sm">{{__('admin_message.Transactions')}}</span> </a>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title">{{__('admin_message.Add Money To service provider Account')}}</h4>
            </div>
            <form action="{{route('transaction.store')}}" method="POST">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                    <label  class="control-label">{{__('admin_message.Service provider')}} *</label>
                            <select style="width:100%" class="form-control select2" name="user_id" required>
                                <option value="">{{__('admin_message.Select')}} {{__('admin_message.Service provider')}}</option>
                                @foreach ($delegates as $delegate)
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title">{{__('admin_message.Withdraw Money To service provider Account')}}</h4>
            </div>
            <form action="{{route('transaction.store')}}" method="POST">
                <div class="modal-body">
                    @csrf
