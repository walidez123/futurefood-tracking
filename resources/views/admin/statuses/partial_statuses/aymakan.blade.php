<form action="{{route('aymakan_statuses.store')}}" method="POST">
    @csrf
    ...
    <div class="col-md-12 ">
        ...
        <div class="box box-primary" style="padding: 10px;">
            ...
            <div class="box-header with-border">
                <h3 class="box-title"> {{__('app.aymakan_statuses')}}</h3>
            </div><!-- /.box-header -->
            <!-- form start -->
            ...
            <div class="col-md-2">
                ...
                <select id="new_order_id" class="form-control select2" name="new_order_id">
                    @foreach ($statuses as $status)
                        <option {{($aymakan_statuses->new_order_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
            ...
            <div class="col-md-2">
                ...
                <select id="assigned_id" class="form-control select2" name="assigned_id">
                    @foreach ($statuses as $status)
                        <option {{($aymakan_statuses->assigned_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                            {{$status->trans('title')}}</option>
                    @endforeach
                </select>
                @error('assigned_id')
            ...
            <div class="col-md-2">
                ...
                <select id="en_route_id" class="form-control select2" name="en_route_id">
                    @foreach ($statuses as $status)
                        <option {{($aymakan_statuses->en_route_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                            {{$status->trans('title')}}</option>
                    @endforeach
                </select>
                @error('en_route_id')
            ...
            <div class="col-md-2">
                <label for="pending_id">{{ __('app.pending') }}</label>
                <select id="pending_id" class="form-control select2" name="pending_id">
                    @foreach ($statuses as $status)
                        <option {{($aymakan_statuses->pending_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                            {{$status->trans('title')}}</option>
                    @endforeach
                </select>
                @error('pending_id')
            ...
            <div class="col-md-2">
                ...
                <select id="delayed_id" class="form-control select2" name="delayed_id">
                    @foreach ($statuses as $status)
                        <option {{($aymakan_statuses->delayed_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                            {{$status->trans('title')}}</option>
                    @endforeach
                </select>
                @error('delayed_id')
            ...
            <div class="col-md-2">
                ...
                <select id="delivered_id" class="form-control select2" name="delivered_id">
                    @foreach ($statuses as $status)
                        <option {{($aymakan_statuses->delivered_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                            {{$status->trans('title')}}</option>
                    @endforeach
                </select>
                @error('delivered_id')
            ...
            <div class="col-md-2">
                ...
                <select id="closed_id" class="form-control select2" name="closed_id">
                    @foreach ($statuses as $status)
                        <option {{($aymakan_statuses->closed_id == $status->id) ? 'selected' : ''}} value="{{$status->id}}">
                            {{$status->trans('title')}}</option>
                    @endforeach
                </select>
                @error('closed_id')
