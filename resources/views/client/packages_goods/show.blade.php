            ...
            <table id="example1" class="table table-bordered table-striped">
              ...
              <tbody>
                ...
                <td>{{$count}}</td>
                 ...
                 <th>{{$packages_good->good->trans('title')}}</th>
                 <th>{{$Packages_history->number}}</th>
                 <th>
                  @if($Packages_history->order!=NULL && $Packages_history->type==2 && Auth()->user()->work==4)
                  <a href="{{ route('client-orders.show', $Packages_history->order->id) }}" target="_blank" rel="noopener noreferrer">{{$Packages_history->order->order_id}}</a>
                  @endif
                  @if ($Packages_history->Pickup_order != null && $Packages_history->type==1)
                                                    <a href="{{ route('orders_pickup.show', $Packages_history->Pickup_order->id) }}"
                                                        target="_blank"
