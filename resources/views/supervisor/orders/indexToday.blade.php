...
<div class="content-wrapper">
  ...
  @include('layouts._header-index', ['title' => trans('order.orders'), 'iconClass' => 'fa-truck', 'addUrl' => '', 'multiLang' => 'false'])
          ...
          <div class="box-body">
              ...
              <table id="example1" class="table table-bordered table-striped table-hover data_table table-fixed-header responsive no-wrap">
                            ...
                            <tr>
                                ...
                                <th>@lang('order.change_status')</th>
                            ...
                            @foreach ($orders as $order)
                                <tr>
                                    ...
                                    <td width="30%">
                                        ...
                                        <p><span style="font-weight: bold; display:block">@lang('order.address'):</span><span>{{ $order->receved_address }}{{ $order->receved_address_2 }}</span></p>
                                    </td>
                                    ...
                                    <td>{{$order->receved_name}}</td>
                                     <td>{{$order->receved_phone}} <a href="tel:{{$order->receved_phone}}" style="padding:5px"><i class="fa fa-phone fa-2x"></i></a> <a href="https://api.whatsapp.com/send?phone={{$order->receved_phone}}"  style="padding:5px">
                                      <i class="fa-brands fa-whatsapp" style="color: green"></i> </a></td>
                                    <td>{{$order->amount}}</td>
                                    ...
                                    <td>{{$order->status->title}}</td>
                                        ...
                                        <a href="{{route('supervisororders.edit', $order->id)}}" title="{{ __('order.change_status') }}" class="btn btn-sm btn-warning" style="margin: 2px;"><i class="fa fa-refresh"></i> <span class="hidden-xs hidden-sm">{{ __('order.change_status') }}</span> </a>
                                        @endif
                                          ...
                                          <a href="{{route('supervisororders.show', $order->id)}}" title="{{ __('order.view') }}" class="btn btn-sm btn-info pull-right" style="margin: 2px;"><i class="fa fa-shield" aria-hidden="true"></i>
                                            <span class="hidden-xs hidden-sm">{{ __('order.show_policy') }}</span> </a>
                                      </td>
                                    </tr>
                                <?php $count++ ?>
                            @endforeach
