    ...
    <section class="invoice">
        <div class="container">
            <div class="row">
                    ...
                    <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(100)->generate($order->order_id)) !!}" alt="QR Code" />
                    </div>
                    ...
                    <div class="col-xs-12 col-md-12 col-lg-12 border" style="height: 90px;padding:5px">
                        ...
                        @if($order->consignmentNo) <div class="col-xs-3 col-lg-3 col-md-3 "> @else <div class="col-xs-6 col-lg-6 col-md-6 "> @endif 
                            ...
                            <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(90)->generate(route('track.order',['tracking_id'=>$order->tracking_id]))) !!}" alt="QR Code" />

                        </div>
                        @if($order->consignmentNo)
                            <div class="col-xs-3 col-lg-3 col-md-3 "> 3pl number :
                                <br> {{$order->consignmentNo}}
                                <img src="data:image/png;base64, {!! base64_encode(QrCode::format('png')->size(90)->generate($order->consignmentNo)) !!}" alt="QR Code" />
                    ...
                    <div class="col-xs-12 col-md-12 col-lg-12 text-center border" style="height: 70px; padding:5px">                        
                        @if ( $order->user->work == 4)
                            @if($order->goods)
                                ...
                                <div class="col-xs-6 col-lg-6 col-md-6 "> {{ __("goods.skus")}} : {{$good->goods->SKUS}} |{{$good->goods->trans('title')}} ({{$good->number}}) pices  <br>
                                </div>
                                @endforeach
                             @endif
